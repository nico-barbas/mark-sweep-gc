...
init_gc_allocator :: proc(gc: ^Gc_Allocator, opt: Gc_Allocator_Options) {
	...
	init_free_list_allocator(&gc.backing, backing_buf, .Find_Best, 8)
...
gc_allocator_proc :: proc(
	...
	result, err = free_list_allocator_proc(
		&gc.backing,
		mode,
		size,
		alignment,
...
collect_garbage :: proc(gc: ^Gc_Allocator) {
	...
	for entry in garbage {
		allocator := free_list_allocator(&gc.backing)
...
gc_used_memory_size :: proc(gc: ^Gc_Allocator) -> int {
	return gc.backing.used
}
